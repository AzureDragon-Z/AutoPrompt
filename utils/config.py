...
def get_llm(config: dict):
    ...
    #     )
    elif config['type'].lower() == 'siliconflow':
        return ChatOpenAI(
            temperature=temperature,
            model_name=config['name'],  # 比如 "deepseek-ai/DeepSeek-R1"
            openai_api_key=config.get('openai_api_key', LLM_ENV['siliconflow']['SILICONFLOW_API_KEY']),
            openai_api_base=config.get('openai_api_base', LLM_ENV['siliconflow']['SILICONFLOW_API_BASE']),
            model_kwargs=model_kwargs
        )
